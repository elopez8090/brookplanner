...
function buildBoroughCategoryHref(
  categorySlug: string,
  boroughSlug: string,
...
export function buildBoroughCategoryIntro(category: VendorCategoryPageConfig, borough: BoroughPageConfig): string {
  return `Find ${category.directoryCategoryName} who serve ${borough.displayName} events. Compare public profiles with bios and service areas, then post your event on Brook Planner to request tailored quotes.`;
}

function categoryHubWithParams(
  slug: string,
  params: { area?: string; q?: string; sort?: VendorDirectorySort },
): string {
  const u = new URLSearchParams();
  if (params.area) {
    u.set("area", params.area);
  }
  if (params.q) {
    u.set("q", params.q);
  }
  ...
  const qs = u.toString();
  return qs ? `${vendorCategoryHubPath(slug)}?${qs}` : vendorCategoryHubPath(slug);
}

export function buildBoroughCategoryMetaDescription(
  category: VendorCategoryPageConfig,
  borough: BoroughPageConfig,
): string {
  return `Find ${category.directoryCategoryName} serving ${borough.displayName} and NYC events. Compare public vendor profiles on Brook Planner, then post your event to request quotes.`;
}

type VendorCategoryBoroughHubProps = {
  category: VendorCategoryPageConfig;
  borough: BoroughPageConfig;
  query: string;
...
export async function VendorCategoryBoroughHub({ category, borough, query, sort }: VendorCategoryBoroughHubProps) {
  const qNorm = normalize(query);
  const areaNorm = borough.serviceAreaMatch;

  const [vendors, peersInCategoryAndBorough] = await Promise.all([
  ...
  const spotlightVendors = selectSpotlightDirectoryVendors(peersInCategoryAndBorough, 3);

  const hasBoroughFilters = Boolean(qNorm) || sort !== "name";

  const areaOptions = Array.from(
    new Set(peersInCategoryAndBorough.flatMap((vendor) => parseServiceAreas(vendor.service_areas))),
  ).sort((a, b) => a.localeCompare(b));

  const h1 = `${borough.displayName} ${category.directoryCategoryName}`;
  const boroughPath = vendorCategoryBoroughPath(category.slug, borough.slug);
  const hoods = neighborhoodsForBorough(borough.slug);
    ...
    <>
      <JsonLd
        data={breadcrumbListJsonLd([
          ...
          { name: borough.displayName, path: boroughPath },
        ])}
      />
      ...
      <header className="space-y-5">
        <div className="space-y-3">
          ...
          <p className="max-w-3xl text-sm leading-relaxed text-brand-navy-muted sm:text-base">
            {buildBoroughCategoryIntro(category, borough)}
          </p>
        </div>
        <nav aria-label="Borough page actions" className="flex flex-wrap gap-2">
          <Link
            href="/post-event"
          ...
          <Link
            href={categoryHubWithParams(category.slug, { q: qNorm, sort })}
            className="inline-flex items-center justify-center rounded-xl border border-border-subtle bg-white px-4 py-2.5 text-sm font-semibold text-brand-navy transition-colors hover:bg-slate-50"
          >
            All NYC {category.directoryCategoryName}
          </Link>
        </nav>
      </header>
      ...
      <nav aria-label="Same category, other boroughs" className="rounded-2xl border border-border-subtle bg-white p-4 shadow-[var(--shadow-card)] sm:p-5">
        ...
        <ul className="mt-3 flex flex-wrap gap-2">
          {BOROUGH_PAGES.map((b) => (
            <li key={b.slug}>
              <Link
                href={buildBoroughCategoryHref(category.slug, b.slug, { q: qNorm, sort })}
                className={
                  b.slug === borough.slug
      ...
      {hoods.length ? (
        <nav
          aria-label={`${borough.displayName} neighborhood guides`}
          className="rounded-2xl border border-border-subtle bg-white p-4 shadow-[var(--shadow-card)] sm:p-5"
        >
          <p className="text-xs font-semibold uppercase tracking-wide text-brand-navy-muted">
            {category.directoryCategoryName} by neighborhood in {borough.displayName}
          </p>
          <ul className="mt-3 flex flex-wrap gap-2">
            {hoods.map((n) => (
              <li key={n.slug}>
                <Link
                  href={vendorCategoryNeighborhoodPath(category.slug, borough.slug, n.slug)}
      ...
      <nav aria-label="Browse other categories" className="rounded-2xl border border-border-subtle bg-white p-4 shadow-[var(--shadow-card)] sm:p-5">
        <p className="text-xs font-semibold uppercase tracking-wide text-brand-navy-muted">Other categories</p>
        <ul className="mt-3 flex flex-wrap gap-2">
          {VENDOR_CATEGORY_PAGES.filter((c) => c.slug !== category.slug).map((c) => (
            <li key={c.slug}>
              <Link
                href={buildBoroughCategoryHref(c.slug, borough.slug, { q: qNorm, sort })}
                className="inline-flex rounded-full border border-border-subtle bg-slate-50 px-3 py-1 text-xs font-medium text-brand-navy transition-colors hover:border-accent-blue/40 hover:bg-white"
              >
                {borough.displayName} {c.directoryCategoryName}
              </Link>
            </li>
          ))}
        </ul>
      ...
      <section aria-label="Search vendors in this borough" className="rounded-2xl border border-border-subtle bg-white p-4 shadow-[var(--shadow-card)] sm:p-5">
        <form
          action={vendorCategoryBoroughPath(category.slug, borough.slug)}
          method="get"
          className="grid grid-cols-1 gap-3 md:grid-cols-3 md:items-end"
        >
          <div className="min-w-0 md:col-span-2">
            <label htmlFor="borough-q" className="block text-xs font-semibold uppercase tracking-wide text-brand-navy-muted">
              Keyword search
            </label>
            <DebouncedUrlKeywordInput
              ...
              name="q"
              initialValue={qNorm}
              placeholder={`Search ${borough.displayName} vendors`}
              className="mt-1.5 w-full min-w-0 rounded-xl border border-border-subtle bg-white px-3 py-2 text-sm text-brand-navy"
            />
          </div>
          <div className="min-w-0">
            <label htmlFor="borough-sort" className="block text-xs font-semibold uppercase tracking-wide text-brand-navy-muted">
              Sort
            </label>
            <select
              id="borough-sort"
          ...
          <div className="flex min-w-0 w-full flex-col gap-2 md:col-span-3 sm:flex-row sm:flex-wrap sm:items-stretch">
            ...
            <Link
              href={categoryHubWithParams(category.slug, { area: areaNorm, q: qNorm, sort })}
            ...
            >
              Filter all service areas on NYC hub →
            </Link>
          </div>
        </form>
        {areaOptions.length > 1 ? (
          <p className="mt-3 text-xs text-brand-navy-muted">
            Vendors here list multiple service areas; results still match <span className="font-semibold text-brand-navy">{borough.displayName}</span> in
            their profile text.
          </p>
        ) : null}
      </section>
      ...
      {spotlightVendors.length ? (
        <section aria-labelledby={`${category.slug}-${borough.slug}-spotlight-heading`} className="space-y-4">
          <div>
            <h2 id={`${category.slug}-${borough.slug}-spotlight-heading`} className="text-lg font-semibold text-brand-navy">
              Spotlight in {borough.displayName}
            </h2>
            <p className="mt-1 text-sm text-brand-navy-muted">
              Vendors with strong marketplace profiles in this category and area.
            </p>
          </div>
      ...
      <section aria-labelledby={`${category.slug}-${borough.slug}-vendors-heading`} className="space-y-4">
        <div className="flex flex-col gap-1 sm:flex-row sm:items-end sm:justify-between sm:gap-3">
          <h2 id={`${category.slug}-${borough.slug}-vendors-heading`} className="text-lg font-semibold text-brand-navy">
            {category.directoryCategoryName} serving {borough.displayName}
          </h2>
          <p className="text-sm text-brand-navy-muted">{marketplaceBoroughVendorsResultLabel(vendors.length, hasBoroughFilters)}</p>
        </div>

        {vendors.length === 0 ? (
          <article className="rounded-2xl border border-border-subtle bg-white p-8 text-center shadow-[var(--shadow-card)]">
            <h3 className="text-lg font-semibold text-brand-navy">
              {hasBoroughFilters ? "No vendors match your filters" : "No vendors match right now"}
            </h3>
            <p className="mt-2 text-sm text-brand-navy-muted">
              Try the NYC-wide {category.directoryCategoryName} hub, browse all vendors, or check another borough.
            </p>
            <div className="mt-4 flex flex-wrap justify-center gap-2">
              <Link
                href={buildBoroughCategoryHref(category.slug, borough.slug, {})}
                className="inline-flex items-center justify-center rounded-xl border border-border-subtle bg-white px-4 py-2.5 text-sm font-semibold text-brand-navy transition-colors hover:bg-slate-50"
              >
                Clear search
              </Link>
              <Link
                ...
                className="inline-flex items-center justify-center rounded-xl bg-brand-navy px-4 py-2.5 text-sm font-semibold text-white transition-colors hover:bg-[#0a2f4f]"
              >
                View all NYC {category.directoryCategoryName}
              </Link>
            </div>
          </article>
        ) : (
